...
then
	genkernel $[jmd/stage5-kde/genkernel/opts:lax] --cachedir=/var/tmp/cache/genkernel all || exit 1
else
	genkernel $[jmd/stage5-kde/genkernel/opts:lax] all || exit 1
fi

sed -i -e '/^c/s!agetty 38400!mingetty --autologin root --noclear!' \
	/etc/inittab || exit 1

for i in $[jmd/stage5-kde/services:zap]; do ln -s /etc/init.d/$i /etc/runlevels/default; done
]
